...
public class OrderServiceImpl implements OrderService {
    ...
    public MyRecord preOrder(PreOrderRequest request) {
        ...
        if (orderInfoVo.getOrderDetailList().get(0).getProductType().equals(Constants.PRODUCT_TYPE_NORMAL)) {
            // 普通商品
            totalPrice = orderInfoVo.getOrderDetailList().stream()
                    .map(e -> e.getVipPrice().multiply(new BigDecimal(e.getPayNum())))
...
//        storeOrder.setIsChannel(isChannel);
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (storeOrder.getSeckillId() > 0) {// 秒杀扣库存
                ...
                // 普通商品口库存
                storeProductService.operationStock(skuRecord.getInt("productId"), skuRecord.getInt("num"),
                        "sub", skuRecord.getInt("productVersion"));
                // 普通商品规格扣库存
                storeProductAttrValueService.operationStock(skuRecord.getInt("attrValueId"), skuRecord.getInt("num"),
                        "sub", Constants.PRODUCT_TYPE_NORMAL, skuRecord.getInt("attrValueVersion"));
                // 秒杀商品规格扣库存
                storeProductAttrValueService.operationStock(skuRecord.getInt("activityAttrValueId"),
                        skuRecord.getInt("num"), "sub", Constants.PRODUCT_TYPE_SECKILL,
                        skuRecord.getInt("activityAttrValueVersion"));
                // 秒杀商品扣库存
                ...
                // 砍价商品规格扣库存
                storeProductAttrValueService.operationStock(skuRecord.getInt("activityAttrValueId"),
                        skuRecord.getInt("num"), "sub", Constants.PRODUCT_TYPE_BARGAIN,
                        skuRecord.getInt("activityAttrValueVersion"));
                // 砍价商品扣库存
                ...
                // 拼团商品规格扣库存
                storeProductAttrValueService.operationStock(skuRecord.getInt("activityAttrValueId"),
                        skuRecord.getInt("num"), "sub", Constants.PRODUCT_TYPE_PINGTUAN,
                        skuRecord.getInt("activityAttrValueVersion"));
                // 拼团商品扣库存
    ...
    private List<MyRecord> validateProductStock(OrderInfoVo orderInfoVo, User user) {
        ...
        if (orderInfoVo.getSeckillId() > 0) {
            ...
            StoreProductAttrValue seckillAttrValue = storeProductAttrValueService
                    .getByIdAndProductIdAndType(detailVo.getAttrValueId(), seckillId, Constants.PRODUCT_TYPE_SECKILL);
            if (ObjectUtil.isNull(seckillAttrValue)) {
                throw new CrmebException("秒杀商品规格不存在");
            }
        ...
        if (orderInfoVo.getBargainId() > 0) {
            ...
            StoreProductAttrValue bargainAttrValue = storeProductAttrValueService
                    .getByIdAndProductIdAndType(detailVo.getAttrValueId(), bargainId, Constants.PRODUCT_TYPE_BARGAIN);
        ...
        if (orderInfoVo.getCombinationId() > 0) {
            ...
            StoreProductAttrValue combinationAttrValue = storeProductAttrValueService
                    .getByIdAndProductIdAndType(detailVo.getAttrValueId(), combinationId, Constants.PRODUCT_TYPE_PINGTUAN);
        ...
        orderDetailList.forEach(e -> {
            ...
            // 查询商品规格属性值信息
            StoreProductAttrValue attrValue = attrValueService
                    .getByIdAndProductIdAndType(e.getAttrValueId(), e.getProductId(), Constants.PRODUCT_TYPE_NORMAL);
    ...
    private MyRecord commonValidateSeckill(StoreSeckill storeSeckill, StoreProductAttrValue seckillAttrValue, User user, Integer productNum) {
        ...
        // 主商品sku
        StoreProductAttrValue productAttrValue = storeProductAttrValueService
                .getByProductIdAndSkuAndType(storeSeckill.getProductId(), seckillAttrValue.getSuk(), Constants.PRODUCT_TYPE_NORMAL);
            ...
            if (storeCouponUser.getUseType().equals(1)) {
                ...
                if (storeCouponUser.getUseType() == 2) {
                    ...
                    List<OrderInfoDetailVo> proOrderDetailList = orderDetailList.stream()
                            .filter(e -> primaryKeyIdList.contains(e.getProductId())).collect(Collectors.toList());
                    ...
                    if (orderInfoVo.getProTotalFee().compareTo(productTotalPrice) <= 0
                            && productTotalPrice.compareTo(storeCouponUser.getMoney()) <= 0) {
                        priceResponse.setDeductionPrice(BigDecimal.ZERO);
                        priceResponse.setSurplusIntegral(user.getIntegral());
                        priceResponse.setPayFee(priceResponse.getFreightFee());
                        priceResponse.setUsedIntegral(0);
                ...
                if (storeCouponUser.getUseType() == 3) {
                    ...
                    //计算金额
                    List<OrderInfoDetailVo> proOrderDetailList = orderDetailList.stream()
                            .filter(e -> containsIdList.contains(e.getProductId())).collect(Collectors.toList());
                    BigDecimal productTotalPrice = proOrderDetailList.stream()
                            .map(e -> e.getVipPrice().multiply(new BigDecimal(e.getPayNum().toString())))
                            .reduce(BigDecimal.ZERO, BigDecimal::add);
                    if (productTotalPrice.compareTo(storeCouponUser.getMoney()) <= 0) {
