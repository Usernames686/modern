...
public class UserStatisticsServiceImpl implements UserStatisticsService {
    ...
    public UserOverviewResponse getOverview(String dateLimit) {
        ...
        if (Constants.SEARCH_DATE_LATELY_7.equals(dateLimit)) {
            // 7天
            // 分别获取今天、前6天的数据，数据相加
        ...
        if (Constants.SEARCH_DATE_WEEK.equals(dateLimit)) {
            // 周
            // 判断今天是否是本周第一天
        ...
        if (Constants.SEARCH_DATE_MONTH.equals(dateLimit)) {
            // 月
            // 判断今天是否是本月第一天
        ...
        if (Constants.SEARCH_DATE_YEAR.equals(dateLimit)) {
            // 年
            // 判断今天是否是本年第一天
            DateTime nowDate = DateUtil.date();
            UserOverviewResponse intervalDate = new UserOverviewResponse();
