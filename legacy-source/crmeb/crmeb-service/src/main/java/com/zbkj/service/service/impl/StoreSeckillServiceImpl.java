...
//        detailH5Response.setStoreInfo(storeSeckill.getInfo());
        ...
        if (ObjectUtil.isNotNull(seckillManger)) {
            int secKillEndSecondTimestamp = CrmebDateUtil.getSecondTimestamp(CrmebDateUtil.nowDateTime("yyyy-MM-dd "
                    + seckillManger.getEndTime() + ":00:00"));
...
//        infoResponse.setStoreInfo(storeSeckill.getInfo());
        ...
        List<StoreProductAttrValue> seckillAttrValueList = storeProductAttrValueService
                .getListByProductIdAndType(skillId, Constants.PRODUCT_TYPE_SECKILL);
        // 查询主商品sku
        List<StoreProductAttrValue> attrValueList = storeProductAttrValueService
                .getListByProductIdAndType(storeSeckill.getProductId(), Constants.PRODUCT_TYPE_NORMAL);
    ...
    public StoreSeckill getByIdException(Integer id) {
        ...
        StoreSeckill storeSeckill = dao.selectOne(lqw);
        if (ObjectUtil.isNull(storeSeckill) || storeSeckill.getIsDel())
            throw new CrmebException("秒杀商品不存在或以删除");
        return storeSeckill;
    }
