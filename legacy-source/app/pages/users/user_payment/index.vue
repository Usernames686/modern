		...
		<form @submit="submitSub" report-submit='true'>
			...
			<view class="payment">
				...
				<view class='tip picList' v-if='!active'>
					<view class="pic-box pic-box-color acea-row row-center-wrapper row-column" :class="activePic === index ?
					'pic-box-color-active' : ''" v-for="(item, index) in picList" :key="index" @click="picCharge(index, item)">
						<view class="pic-number-pic">
							{{ item.price }}<span class="pic-number"> 元</span>
						</view>
						<view class="pic-number">赠送：{{ item.giveMoney }} 元</view>
					</view>
					<view class="pic-box pic-box-color acea-row row-center-wrapper" :class="parseFloat(activePic)===parseFloat(picList.length)?
					'pic-box-color-active':''" @click="picCharge(picList.length)">
						<input type="number" placeholder="其他" v-model="money" maxlength="5"
				...
				<view class="tip" v-else>
					...
					<view class="tips-title">
						...
						<view style="margin-top: 10rpx;">当前佣金为 <text
								class='font-color'>￥{{userInfo.brokeragePrice || 0}}</text></view>
					</view>
					<view class="tips-box">
						<view class="tips mt-30">注意事项：</view>
						<view class="tips-samll" v-for="item in rechargeAttention" :key="item">
							...
							appWechat({
								...
								uni.requestPayment({
									...
									success: function(res) {
										that.$store.commit("changInfo", {
											amount1: 'nowMoney',
											amount2: that.$util.$h.Add(value, that
												.userInfo.nowMoney)
										});
										return that.$util.Tips({
											title: '支付成功',
								...
								if (that.from == "weixinh5") {
									...
									that.$wechat.pay(data)
										.finally(() => {
											that.$store.commit("changInfo", {
												amount1: 'nowMoney',
												amount2: that.$util.$h.Add(value, that.userInfo
													.nowMoney)
											});
											return that.$util.Tips({
												title: '支付成功',
