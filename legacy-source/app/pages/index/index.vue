	...
	<view id="home" :data-theme="theme" :style="[pageStyle]">
		...
		<view v-if="!errorNetwork">
			<view class="page-index tui-skeleton page_count" :class="{'bgf':navIndex >0}"
				...
				<!-- 组合组件 -->
				<homeComb v-if="showHomeComb" :bgInfo="bgInfo" :dataConfig="homeCombData" @changeTab="changeTab"
				...
				<view class="page_content skeleton">
					<view v-if="navIndex === 0">
						<view v-for="(item, index) in styleConfig" :key="index">
							...
							<!-- 商品列表 -->
							<goodList v-if="item.name == 'goodList'&&!item.isHide" :dataConfig="item"
								@detail="goDetail"></goodList>
							<!-- 选项卡商品列表-->
							<homeTab v-if="item.name == 'homeTab'&&!item.isHide" :dataConfig="item" @detail="goDetail">
							</homeTab>
						</view>
					</view>
