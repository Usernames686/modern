<template>
	<view :data-theme="theme">
		...
		<view class='bargain skeleton' :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
			<view class="header" :class="startBargainUid != uid ? 'on' : ''"
				:style="{backgroundImage:'url('+imgHost+ '/' + backBg+')'}" v-show="imgHost">
				<!-- #ifdef H5 -->
			...
			<view class="pad30" style="padding-bottom: 30rpx;">
				...
				<view class="content">
					...
					<view
						v-if="startBargainUid != uid && bargainStatus==4 && parseFloat(bargainUserInfo.surplusPrice) == 0">
				...
				<view class='bargainTip' :class='active==true?"on":""'>
					<view class='pictrue' :style="{backgroundImage:'url('+imgHost +'/'+backHead+')'}" v-show="imgHost">
					</view>
		...
		onLoad: function(options) {
			...
			if (this.isLogin) {
				...
				this.$Cache.set(BACK_URL,
					...
					'&startBargainUid=' + this.uid + '&spread=' + this.uid + '&storeBargainId=' + this
					.storeBargainId
				);
				toLogin();
			}
