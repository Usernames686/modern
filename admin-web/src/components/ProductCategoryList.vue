<template>
  <div class="divBox">
    ...
    <el-card class="box-card mt14">
      <template #header>
        <div class="clearfix">
          <el-button type="primary" size="small" @click="openCreate({ id: 0, name: '顶层目录' })">添加商品分类</el-button>
        </div>
      </template>
      ...
      >
        ...
        <el-table-column label="操作" width="210" fixed="right">
          <template #default="{ row }">
            <el-button v-if="row.pid === 0" link type="primary" @click="openCreate(row)">添加子目录</el-button>
            <el-divider v-if="row.pid === 0" direction="vertical" />
    ...
    <el-dialog
      ...
      :title="dialog.mode === 'create' ? '添加商品分类' : '编辑商品分类'"
      width="540px"
      destroy-on-close
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="130px">
        <el-form-item label="分类名称：" prop="name">
          <el-input v-model="form.name" maxlength="8" placeholder="分类名称" />
        </el-form-item>
        <el-form-item label="父级：" prop="pid">
          <el-cascader
            v-model="form.pid"
            :disabled="dialog.mode === 'edit' && Number(form.pid) === 0"
...
const rules = {
  ...
  pid: [{ required: true, message: '请选择父级分类', trigger: ['blur', 'change'] }],
...
const parentOptions = computed(() => [
  ...
  ...disableSecondLevel(structuredCloneSafe(treeList.value), form.id)
]);
...
function disableSecondLevel(list, editingId) {
